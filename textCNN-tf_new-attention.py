...
ft = fasttext.load_model("bak/br_word2v.bin")
...
space = ft.get_word_vector(" ")

def get_input_vector(query):
  word_sequences = [text.split() for text in query]
  ret = []
  for line in word_sequences:
    vec_line = []
    for i in range(0, MAX_SEQUENCE_LENGTH):
      if i < len(line):
        vec_line.append(ft.get_word_vector(line[i]))
      else:
        vec_line.append(space)
    ret.append(vec_line)
  return np.asarray(ret)

x_train = get_input_vector(x_train_ori)
x_test = get_input_vector(x_test_ori)
...
def TextCNN_model_2(x_train, y_train, x_val,  y_val):
    main_input = tf.keras.layers.Input(shape=(MAX_SEQUENCE_LENGTH, EMBEDDING_DIM), dtype='float', name='input_1')
    ...
    attention = tf.keras.layers.Attention(use_scale=True)([main_input, main_input, main_input])
    attention = attention + main_input
    main_input_norm = tf.keras.layers.LayerNormalization()(attention)

    # 卷积核大小分别为2,3,4
    cnn1 = tf.keras.layers.Conv1D(128, 1, padding='valid', strides=1, activation='relu')(main_input_norm)
    cnn1 = tf.keras.layers.MaxPooling1D(pool_size=MAX_SEQUENCE_LENGTH - 1 + 1)(cnn1)
    #cnn2 = tf.keras.layers.Conv1D(128, 2, padding='valid', strides=1, activation='relu')(main_input_norm)
    #cnn2 = tf.keras.layers.MaxPooling1D(pool_size=MAX_SEQUENCE_LENGTH - 2 + 1)(cnn2)
    cnn3 = tf.keras.layers.Conv1D(128, 3, padding='valid', strides=1, activation='relu')(main_input_norm)
    ...
    #model.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=10, batch_size=1, callbacks=[model_checkpoint_callback])
...
if TASK_CLASS_NUMBER == 2:
    print(x_train.shape)
    load_model = TextCNN_model_2(x_train, y_train, x_test, y_test)
else:
...
x_dcg = get_input_vector(dcg_query)
